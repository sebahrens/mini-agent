...
((parse, apply) => (original, encode) => encodedArguments => {
    ...
    let result;
    try {
        result = apply(original, undefined, values);
    } catch (_) {
        throw 0;
    }
...
pub(crate) fn load_artifact(
    ...
    let (namespace, bridge_factory, private_encoder) = private_context
        .with(|ctx| {
    ...
    install_model_wrappers(model_context, artifact, bridges)?;
    Ok(LoadedArtifact {
        ...
        .with(|ctx| {
            ...
            let wrappers = artifact
                ...
                .map(|(export, bridge)| {
                    ...
                    Ok((export.name.clone(), wrapper))
                })
                .collect::<rquickjs::Result<Vec<_>>>()?;

            // Publish only after every wrapper has been constructed successfully.
            for (name, wrapper) in wrappers {
                ctx.globals()
                    .prop(name.as_str(), Property::from(wrapper).enumerable())?;
            }
            Ok::<_, rquickjs::Error>(())
        })
