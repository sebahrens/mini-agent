...
fn worker_protocol_rejects_malformed_unknown_and_wrong_direction_json() {
    ...
    assert!(matches!(
        read_frame::<_, ParentWireFrame>(&mut Cursor::new(framed(
            br#"{"protocol_version":4,"build_id":"test-build-1","invocation_id":null,"sequence":0,"message":{"kind":"future_parent_message"}}"#
        ))),
        Err(FrameError::InvalidJson)
    ));
    assert!(matches!(
...
fn worker_protocol_rejects_unknown_fields_and_invalid_wire_ids() {
    let payload = br#"{"protocol_version":4,"build_id":"test-build-1","invocation_id":null,"sequence":0,"message":{"kind":"shutdown"},"smuggled":"value"}"#;
    let mut bytes = (payload.len() as u32).to_be_bytes().to_vec();
    bytes.extend_from_slice(payload);
    assert!(matches!(
    ...
    for hello in [
        br#"{"protocol_version":4,"build_id":"test-build-1","invocation_id":null,"sequence":0,"message":{"kind":"hello","data":{}}}"#.as_slice(),
        br#"{"protocol_version":4,"build_id":"test-build-1","invocation_id":null,"sequence":0,"message":{"kind":"hello","data":{"challenge":"00000000-0000-0000-0000-000000000000"}}}"#.as_slice(),
    ] {
