...
#[tokio::test]
async fn worker_fault_matrix_reuses_only_success_js_errors_and_known_effect_outcomes() {
    ...
    for (code, expected, expected_terminal) in [
        ...
        let effects = if code == "outcome-unknown" {
            MatrixEffects::outcome_unknown()
        } else {
            MatrixEffects::default()
        };
...
async fn worker_fault_matrix_bounds_process_call_count_and_age() {
    ...
    let supervisor = JsWorkerSupervisor::with_launcher_and_policy_for_test(
        launcher.clone(),
        Duration::from_secs(2),
        WorkerReusePolicy::new(Duration::from_millis(20), 32),
    );
    call(&supervisor, "success", MatrixEffects::default())
        .await
        .unwrap();
