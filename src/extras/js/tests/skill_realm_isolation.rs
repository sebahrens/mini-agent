...
fn bundled_ajv_is_available_only_inside_private_skill_realms() {
    ...
    let model = Context::full(&runtime).expect("create model context");
    let skill = SkillArtifact::new(
        "function inspectAjv(value) { const valid = Ajv.validate({$schema: 'http://json-schema.org/draft-07/schema#', type: 'string'}, value); return {valid, errors: Ajv.errors, facadeFrozen: Object.isFrozen(Ajv), errorsFrozen: Ajv.errors === null || Object.isFrozen(Ajv.errors), constructorHidden: typeof Ajv.constructor === 'undefined'}; }".into(),
