...
fn prepare_verification_bindings(
    ...
    for (export_index, export) in artifact.exports.iter().enumerate() {
        let invocation_id =
            super::protocol::InvocationId::new(format!("verify-{case_index}-{export_index}"))
                .map_err(|_| ())?;
        let authorization = InvocationAuthorization::new(
            invocation_id,
            artifact.id.clone(),
            export.name.clone(),
            artifact.capability.clone(),
            artifact.capability.grants.iter().map(|scope| {
                (
                    scope.capability(),
        ...
        .map_err(|_| ())?;
        let handle = capabilities.prepare(authorization).map_err(|_| ())?;
        let invocation = format!("verify-{case_index}-{export_index}");
        bindings.insert(
            export.name.clone(),
            super::realm::BoundExportInvocation {
                handle,
                on_start: Arc::new(move |_| Ok(invocation.clone())),
