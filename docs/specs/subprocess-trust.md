...
| `TC-LIFECYCLE-HELPER` — `sandbox::kill_process_group` | mini-agent directly invokes the Unix process-signalling API through safe `nix::killpg` calls with fixed `SIGTERM` then `SIGKILL` against a validated child process-group ID. PID is derived from a child started by mini-agent, never arbitrary model text. | Irrelevant; has no executable or cwd dependency. | No child environment exists; signalling is an in-process OS operation. | No file/network access required; needs host process-signalling authority. | Runs at the host lifecycle layer outside the child sandbox so it can terminate the group. It is not callable as a general command profile. | Audit is attached to the owning child termination event; signalling failures other than an already-absent group are logged. | Immediate, synchronous signalling. The direct child is separately awaited by the owning runner. | Unix uses the OS process-group signal primitive; Windows service trees use a Job Object rather than this helper. |
...
| `TC-LIFECYCLE-HELPER` | In-process Unix process-group signalling in `src/sandbox.rs`; it launches no subprocess. |
