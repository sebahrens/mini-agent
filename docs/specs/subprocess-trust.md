...
|---|---|---|---|---|---|---|---|---|
| `TC-MODEL-ACTION` — `BashTool::call`; JS `make_spawn_with_timeout`; shared constructors in `Sandbox::wrap_command`; terminal launch in `Sandbox::run_built_output_command` | Model authors the Bash script or JS `cmd`/`args`. Bash uses the configured shell with opaque `-c` text. JS uses fixed `exec "$0" "$@"` shell text and appends program/arguments as distinct argv entries. | Canonical current process workspace at launch; requested wrappers set it explicitly. | Requested `bwrap`/Seatbelt clears ambient credentials and restores only the Phase 2 allow-list. An explicit unsandboxed choice inherits ambient environment. | Workspace and application cache writes. Requested backends deny IP network; exact backend read/device/process guarantees come from Phase 2. Unsandboxed mode has ambient host authority and must report that fact. | Shared `Sandbox`; never a raw JS launch. `requested-but-unavailable` starts no child and never retries unsandboxed. | Mandatory `bash` permission for the complete Bash script or structural JS program/argv identity; denial/cancellation has no effect. Sandbox capability state is user-visible. **Current gap:** JS joins argv with spaces for permission lookup, so distinct argv vectors can collide and must be replaced by a canonical structural key. | 30 s maximum; 1 MiB stdout, 1 MiB stderr, 1.5 MiB combined; cancellation kills and reaps the process group. | Linux `bwrap` and macOS Seatbelt guarantees are exactly those in Phase 2. Windows has no isolation claim; requested isolation may not be reported as active without a defined backend. |
...
| `TC-LOOP-VALIDATION` — `run_headless_loop` and interactive `handle_agent_response` | Human authors `--loop-run` opaque shell text. mini-agent selects `sh -c` or PowerShell `-Command`; model output never becomes this command. | Current process workspace at launch. | Ambient environment/credentials because validation commonly invokes project toolchains. | Workspace/project files and whatever network the explicitly selected validation tool needs. | Not the model-action sandbox and never the broker-only worker profile. Any requested backend is fail-closed. | CLI configuration is the authorization event. Audit must identify the command, iteration, cwd, and outcome. **Current gap:** no shared audit event. | Must have finite time/output limits, cancellation, reaping, and tree cleanup. **Current gap:** both duplicate sites use unbounded `.output()`. | `sh` on Unix/macOS and PowerShell on Windows; lifecycle parity is not yet established. |
...
| `TC-SUPPORT-UTILITY` — `show_get_started` (`less`), `open_url`, `copy_to_clipboard`, `InputEditor::open_in_editor`, `edit_memory_file_with_shell`, and `App::run_lazygit` | mini-agent selects pager/opener/clipboard/lazygit argv; human selects URL/action and may configure opaque editor shell text. These are explicit local UI actions, not model commands. | Startup workspace, except editor operates on a generated temporary path. | Ambient desktop/session environment and credentials where the utility requires them. | Selected document/temp/workspace plus desktop IPC; browser may use network. | No broker containment. A desktop utility profile may be unsupported in headless mode; requested restriction never silently falls back. | The initiating UI action authorizes launch. Audit requires utility kind, sanitized target identity, cwd, and outcome; never clipboard/editor contents. **Current gap:** no shared audit. | Interactive utilities may run until human exit but still require caller cancellation and direct-child/tree reaping. Probe/clipboard/opener attempts require short bounds. **Current gap:** raw `.spawn/.status/.output` sites have no common lifecycle policy. | Platform utility lists differ intentionally. Missing tools fall through where documented; unsupported interactive/headless combinations must be visible. |
...
as `TEST-ONLY`/`NON-PROCESS`, and assigns every remaining match to one class above. It counts
identical fingerprints per file, so adding a second identical `Command::new("git")` still fails.
A new or changed match fails the default test until its owner audits the full launch contract,
updates this normative table when needed, and adds the classification reference. Removing a site
also fails until stale inventory is removed.
...
| `TC-SUPPORT-UTILITY` | Process sites in `src/docs.rs`, lazygit in `src/ui/app.rs`, editor sites in `src/ui/input/mod.rs` and `src/ui/slash/memory.rs`, and opener/clipboard sites in `src/ui/renderer.rs`. |
...
- [x] The broker-only JavaScript worker is distinct from workspace services, explicit user
      commands, and the current shared model-action sandbox.
