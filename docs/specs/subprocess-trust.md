...
| `NON-PROCESS` | Lexical exclusions in ACP, export/HTTP, JS runtime/skill/supervisor stderr-drain and verification-dispatch thread tasks, fake verification, source comments, and assertions that legacy loop callers contain no raw Tokio process constructor. |
