...
|---|---|---|---|---|---|---|---|---|
| `TC-MODEL-ACTION` — `BashTool::call`; JS `make_spawn_with_timeout`; shared constructors in `Sandbox::wrap_command`; terminal launch in `Sandbox::run_built_output_command` | Model authors the Bash script or JS `cmd`/`args`. Bash uses the configured shell with opaque `-c` text. JS uses fixed `exec "$0" "$@"` shell text and appends program/arguments as distinct argv entries. | Canonical session workspace selected at agent construction; ACP validates the canonical `session/new` identity and then enters a retained directory handle (`fchdir` on Unix), while other frontends use their startup workspace. Every wrapper sets child CWD explicitly without process-global mutation. | Requested `bwrap`/Seatbelt clears ambient credentials and restores only the Phase 2 allow-list. `zerobox` environment behavior is backend-defined and unknown to mini-agent. An explicit unsandboxed choice inherits ambient environment. | Requested `bwrap` and Seatbelt deny IP network and enforce their Phase 2 filesystem policies. ACP wrappers derive their workspace mount/policy path from an inherited directory descriptor so a pathname race cannot redirect the effect. `zerobox` cannot consume that authority and therefore fails closed for ACP on Unix. Unsandboxed mode has ambient host authority. | Shared `Sandbox`; never a raw JS launch. `requested-but-unavailable` starts no child and never retries unsandboxed. `zerobox` must not be reported as satisfying the `bwrap` or Seatbelt capability matrix. | Mandatory `bash` permission for the complete Bash script or structural JS program/argv identity; denial/cancellation has no effect. Sandbox capability state is user-visible. **Current gap:** JS joins argv with spaces for permission lookup, so distinct argv vectors can collide and must be replaced by a canonical structural key. | 30 s maximum; 1 MiB stdout, 1 MiB stderr, 1.5 MiB combined; cancellation kills and reaps the process group. | Linux `bwrap` and macOS Seatbelt guarantees are exactly those in Phase 2. `zerobox` has only backend-defined, capability-reported behavior on every platform. Windows has no isolation claim; requested isolation may not be reported as active without a defined backend. |
...
| `TC-LSP-SERVICE` — `LspClient::spawn` | Built-in registry or trusted user LSP configuration authors program, direct args, and env overrides; binary resolves through PATH. Model edits trigger lazy startup but do not author argv. | Resolved session project root, set explicitly on the child. ACP validates its captured root, gives the Unix child an inherited directory descriptor, and uses the descriptor path for `rootUri`; Windows holds the root name against deletion. | Ambient environment/credentials plus configured overrides, because language servers need project toolchains. Secret overrides must never be logged. | Full project/workspace reads and common build-cache writes; network may be needed for toolchains. ACP file synchronization opens the relative file through the session capability and reads the stable, no-follow handle. | Dedicated workspace-service profile, never the broker-only worker profile. Requested service containment fails closed. **Current state:** no OS sandbox. | Enabling trusted LSP config authorizes launch; audit identifies server name, executable identity, root, and lifecycle outcome. **Current gap:** trace logging is not a complete audit record. | 15 s initialize request deadline and `kill_on_drop`/`start_kill`. **Current gaps:** stderr is unbounded, drop does not wait/reap, and descendant cleanup is not defined (`mini-agent-7r1a.4`). | Spawn is cross-platform; Unix/Windows tree cleanup and resource parity are unverified. |
