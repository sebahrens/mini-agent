...
|---|---|---|---|---|---|---|---|---|
| `TC-MODEL-ACTION` — `BashTool::call`; brokered JS `SpawnEffectService`; shared constructors in `Sandbox::wrap_command`; terminal launch in `Sandbox::run_built_output_command` | Model authors the Bash script or JS `cmd`/`args`. Bash uses the configured shell with opaque command text. Unix JS uses fixed `exec "$0" "$@"`; Windows JS sends structural program/argv through the inherited helper-request handle. | Canonical current process workspace at launch; every supported wrapper sets it explicitly. | Requested `bwrap`, Seatbelt, and AppContainer launch clear ambient credentials and restore only their documented narrow allow-lists. The Windows candidate is production-unavailable pending hosted proof. `zerobox` is backend-defined. An explicit unsandboxed choice inherits ambient environment. | Linux/macOS deny IP network and enforce their Phase 2 filesystem policies. The Windows candidate grants the canonical workspace, application cache, exact selected executable, and bounded AppContainer-scoped configured roots; host writes are workspace plus explicit write roots, and network capabilities are empty. `zerobox` is backend-defined. | Shared `Sandbox`; never a raw JS launch. The hosted Windows candidate uses a fixed-function same-executable helper, unique AppContainer, handle-bound ACLs, locked executable proof, private desktop, and creation-time UI-restricted Job; it is separate from LPAC. Requested-unavailable never retries unsandboxed. | Mandatory `bash` permission for the complete Bash script or structural JS program/argv identity; denial/cancellation has no effect. Sandbox capability state is user-visible. | 30 s maximum; bounded output; cancellation kills and drains the complete Linux namespace/process group or macOS process group. The hosted Windows candidate waits for the exact Job to report zero active processes before ACE/profile cleanup; stale recovery validates a parent-only named Job, retaining its recovery journal on uncertain state or any cleanup failure. | Linux and macOS guarantees are exactly those in Phase 2. Windows availability remains pending its native explicit-root/network/Job attestation and cached production preflight. |
...
| `TC-BROKER-JS-WORKER` — `sandbox::worker::macos` | mini-agent selects the canonical same-executable worker image and fixed internal marker; no model/user argv is accepted. | Would require a private broker-only root with no workspace authority. | Would clear the environment and restore only the fixed marker. | No production authority is granted because the backend is unavailable. | The real Seatbelt probe shows that allowing the stable image for initial execution also leaves it reusable by a native-compromised worker; a second tighter profile cannot close the transition. Production returns unavailable and never retries under deprecated best-effort Seatbelt. | Typed backend/assurance/reason status only; no arbitrary worker data. | No worker starts, so no resource or teardown guarantee is claimed. | macOS: deliberately unavailable until a standalone mechanism closes reusable exec. The negative probe is evidence, not containment success. |
| `TC-BROKER-JS-WORKER` — `sandbox::worker::windows` production launch and attestation | mini-agent selects the supported current executable, fixed internal marker, and exact anonymous protocol handles. No model/user argv is accepted. | LPAC process with no workspace cwd authority. | Unicode environment block contains only the fixed worker marker; credentials are absent. | Zero-capability LPAC is a token boundary, not a filesystem namespace: host objects whose ACLs grant the stable package identity access can remain visible. The minimal local attestation does not test ambient workspace/skill-store, credential, loopback-network, actual-child, omitted-handle, or install-root denial. Hosted canaries record those observations only on their reference runner. | Exact `HANDLE_LIST`, compatible mitigations, creation-time Job assignment, active-process limit one, memory/CPU/UI limits, and kill-on-close. A process-wide `OnceLock` reports available only after the same-launcher attestation verifies parent Job/mitigation state, child LPAC/token/handle/console/device state, a closed protocol probe, `6 * 7 == 42`, shutdown, and successful exit. Startup and `--print-config` status preflight create/reuse a persistent AppContainer profile and may add a persistent exact RX ACE to a supported user-owned installed executable; no automatic cleanup, rollback, or consent prompt exists. | Internal worker creation only; parent-brokered effects retain independent grants, permission, and durable audit. The hosted full canary/install matrix is pending reference-runner evidence and does not grant runtime authority. | Job close owns whole-tree termination and reaping. The OS creation call runs on one owned helper thread behind a five-second caller deadline but is not itself cancellable; a late result is torn down and a permanently blocked call cannot trigger a second launcher. | Windows: available only after local attestation. Final hosted and standard-user canary observations are still pending; LPAC worker containment does not authorize general JS `spawn`. |
...
| `TC-LOOP-VALIDATION` — `extras::loop::validation`, called by `run_headless_loop` and interactive `handle_agent_response` | Human authors `--loop-run` opaque shell text. mini-agent selects `sh -c` or PowerShell `-Command`; model output never becomes this command. | Current process workspace at launch, subject to the selected `Sandbox` backend's explicit cwd policy. | The selected `Sandbox` environment contract applies: requested Seatbelt/bwrap/AppContainer containment narrows inherited environment, while an explicit unsandboxed choice inherits ambient toolchain credentials. | Linux/macOS validation deny network; Windows grants the workspace only for host writes and supplies no network capability. Requested containment never retries with ambient authority. | Uses the shared `Sandbox` command lifecycle under the distinct validation trust class and never the broker-only worker profile. Any requested backend is fail-closed. | CLI configuration is the authorization event. Audit must identify the command, iteration, cwd, and outcome. **Current gap:** no shared audit event. | 30 s maximum; bounded output; operation-scoped cancellation reaps the owned process group or Windows Job without cancelling unrelated commands. | `sh` on Unix/macOS and PowerShell on Windows; the Windows AppContainer helper owns the complete descendant tree. |
...
| `TC-INTERNAL-VERIFICATION` — `verify_workflow_only_headless_relevance`; hidden Windows AppContainer runtime probe | mini-agent authors fixed Bash/PowerShell/native probe behavior from checked-in policy; no user/model text enters argv. | Repository/startup workspace for workflow verification; fresh private workspace/cache/outside fixtures for the Windows probe. | Workflow verification still inherits ambient credentials. The Windows probe uses the AppContainer helper's narrow environment and private profile storage. | Workflow verification reads repository policy inputs and needs no network. The Windows probe covers exact-root reads, allowed/denied writes, hard links, path/executable stability, zero capabilities, loopback exemptions, IPv4/IPv6 TCP/UDP denial, launcher-token acquisition, omitted-handle denial, descendant AppContainer/Job retention, and crash-stale cleanup. | Internal verifiers, not model or broker authority. The Windows probe traverses the production helper, AppContainer attribute, handle ACLs, private desktop, creation-time Job, and parent-death path; failures are terminal. **Current workflow gap:** raw unsandboxed Bash. | A fixed hidden verification flag is the audit identity; record policy version and outcome. | The workflow `.status()` remains unbounded. The Windows probe is CI-bounded; capped request feeders and the production kill-on-close Job own its descendants, and cleanup attests profile/store/journal/ACE removal. | Workflow verification is Unix-only. The Windows probe is Windows-only and prints one exact bounded, source-free pass record. |
...
empty-root bubblewrap path. macOS reports the reusable-exec Seatbelt blocker as unavailable.
