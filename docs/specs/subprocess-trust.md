...
|---|---|---|---|---|---|---|---|---|
| `TC-MODEL-ACTION` — `BashTool::call`; brokered JS `SpawnEffectService`; shared constructors in `Sandbox::wrap_command`; terminal launch in `Sandbox::run_built_output_command` | Model authors the Bash script or JS `cmd`/`args`. Bash uses the configured shell with opaque command text. Unix JS uses fixed `exec "$0" "$@"`; Windows JS sends structural program/argv through the inherited helper-request handle. | Canonical current process workspace at launch; every supported wrapper sets it explicitly. | Requested `bwrap`, Seatbelt, and AppContainer launch clear ambient credentials and restore only their documented narrow allow-lists. `zerobox` remains backend-defined. An explicit unsandboxed choice inherits ambient environment; an implicitly unavailable default clears it and restores only the non-credential allow-list. | Linux/macOS deny IP network and enforce their Phase 2 filesystem policies. Windows adds package-SID grants for the canonical workspace, application cache, exact selected executable, and bounded configured roots, while regular-AppContainer resources already readable to `ALL APPLICATION PACKAGES` remain visible; AppContainer network capabilities are empty. `zerobox` is backend-defined. | Shared `Sandbox`; never a raw JS launch. Windows uses a fixed-function same-executable helper, unique AppContainer, handle-bound ACLs, locked executable proof, private desktop, and creation-time UI-restricted Job; it remains separate from LPAC. Requested-unavailable never retries unsandboxed. | Mandatory `bash` permission for the complete Bash script or structural JS program/argv identity; denial/cancellation has no effect. JS approval and policy identities preserve argv boundaries. Sandbox capability state is user-visible. | 30 s maximum with bounded output. Unix kills and drains the owned process group; Windows waits for the exact Job to report zero active processes before ACE/profile cleanup and retains recovery state when safe cleanup cannot be proven. Before a cached Windows startup preflight begins, a five-second bounded sweep recovers only exact, private roots preserved by prior interrupted preflights. The new probe then has a five-second run deadline, up to five seconds for whole-tree reaping, and a fresh five-second profile/ACL recovery ceiling; it synchronously owns its helper and publishes a durable profile intent before profile creation. Windows `CreateProcessW` itself is not cancellable; if that OS call returns after the run deadline, the still-owned child is rejected and reaped before the probe returns. | Linux and macOS guarantees are exactly those in Phase 2. Windows becomes available only after its native explicit-root/network/Job attestation and cached production preflight succeed. |
| `TC-MODEL-ACTION` — structured `GitTool` operations through `GitRunner` and `Sandbox::wrap_workspace_service` | The model selects one fixed `status`, `diff`, `log`, `show`, `stage`, `unstage`, or `commit` operation. Paths are bounded literal repository-relative operands after `--`; revisions are verified object names; commit text is bounded and supplied through stdin. Raw argv, shells, remotes, and network operations are absent. | The identity-pinned Git executable always receives `-C` with the captured canonical `WorkspaceBinding`; repository-redirection environment variables cannot retarget it. | Ambient credentials and Git redirection/config variables are removed. Only the fixed non-credential Git environment and platform runtime variables needed by the contained launcher are present. Hooks, signing, credential helpers, editors, pagers, external diff/textconv, recursive submodules, and file/ext protocols are disabled. | The selected workspace is readable and writable only under the configured general-process sandbox; network is denied. Stage rejects symlink operands and paths with filter or working-tree-encoding transforms. | Production capture verifies the pinned Git executable is launchable through the selected workspace-service sandbox. Requested-unavailable fails before tool registration and there is no uncontained production fallback. | Read operations require their exact `git/*` permission. Mutations require exact `git/stage`, `git/unstage`, or `git/commit` permission before taking the process-wide admission lock. Results expose bounded output plus truthful before/after porcelain-v2 snapshots and a closed terminal status; they never claim rollback. | Queries have 10 s bounds; local mutations have 60 s bounds and 512 KiB per-stream/768 KiB combined output limits. Timeout, cancellation, and output overflow kill and reap the child before the post-operation snapshot is captured. Git's index lock remains authoritative, while one process-wide lock serializes mini-agent mutations around their snapshots. | Direct Git argv and stdin are portable. Unix uses process-group ownership; Windows uses the general AppContainer helper and Job cleanup after its runtime preflight. |
...
`TC-MCP-STDIO`, `TC-LSP-SERVICE`, `TC-EXPLICIT-USER-SHELL`, `TC-LOOP-VALIDATION`, `TC-GOAL-CHECK`, and
...
| `TC-GOAL-CHECK` | Same shared bounded runner, reached from `src/extras/goal/checks.rs`; the goal module constructs no process directly and never calls `Sandbox::wrap_command` itself. |
...
| `TC-SUPPORT-UTILITY` | Direct-argv sites: pager in `src/docs.rs`, lazygit in `src/ui/app.rs`, opener/clipboard constructors in `src/ui/renderer.rs`, and the fixed macOS version probe in `src/sandbox/worker/macos.rs`. Opaque editor-shell sites: `src/ui/input/mod.rs` and `src/ui/slash/memory.rs`. The Windows `ShellExecuteW` URL opener in `src/ui/renderer.rs` is `NON-PROCESS` and has no command-interpreter fallback. |
...
| `NON-PROCESS` | Lexical exclusions in ACP (including permission-result status builders), export/HTTP, JS runtime/skill/supervisor stderr-drain, verification-dispatch, idle-retirement, the session-owned sandbox background-task dispatch, fake verification, source comments, and assertions that legacy loop callers contain no raw Tokio process constructor. |
