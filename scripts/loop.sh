...
run_verification() {
    ...
    local changed_files="" raw_changed_files="" diff_check_output="" relevant_files=0
    if ! git rev-parse "$range_base" "$range_head" >/dev/null 2>&1 \
            || ! raw_changed_files=$(git diff --name-only "$range_base" "$range_head" 2>/dev/null); then
        failed=1; real_failure=1
        append_verification_error "verification range" "Unable to inspect $range_base..$range_head"
    elif [ -z "$raw_changed_files" ]; then
        failed=1
        ...
        # Generated bytecode is never verifier input. Filter it once, before
        # relevance counting and checker dispatch, so it cannot be mistaken for
        # a script implementation that needs its own source checker.
        local candidate_file
        while IFS= read -r candidate_file; do
            path_is_generated_python_bytecode "$candidate_file" && continue
