...
- **Command construction.** Any `Command::new` whose arguments derive from untrusted input. `shell: false` equivalents: prefer `arg()` over building a shell string. `src/extras/hooks/subprocess.rs` deliberately uses `("sh", "-c")` on unix and `("powershell", "-Command")` on Windows — that is a documented design decision, so review the *trust gate in front of it* (`src/extras/hooks/trust.rs`), not the invocation itself.
